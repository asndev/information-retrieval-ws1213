...
void SearchServer::ServerLoop() {
  // Now the server part.
  // std. values for bonudaries, if something breaks.
  size_t overallResults = 10, numHits = 10;
  try {
    ...
    while (true) {
      ...
      cout << endl << "request string is \""
           ...
           << "\"" << endl << endl;
      // Extract number for suggestions and hits.
      size_t pos1 = request.find("numSuggestions=");
      size_t pos2 = request.find("&", pos1);
      if (pos1 != std::string::npos || pos2 != std::string::npos) {
        overallResults = static_cast<size_t>(atoi(request.substr(pos1 + 15,
                                              pos2 - pos1 - 15).c_str()));
        std::cout << "Overall Results: " << overallResults << std::endl;
      }
      pos1 = request.find("numHits=");
      pos2 = request.find("&", pos1);
      if (pos1 != std::string::npos || pos2 != std::string::npos) {
        numHits = static_cast<size_t>(atoi(request.substr(pos1 + 8,
                                              pos2 - pos1 - 8).c_str()));
        std::cout << "Number of max. hits: " << numHits << std::endl;
      }

      // Extract query string.
      std::string query;
      pos1 = request.find("query=");
      pos2 = request.find("&", pos1);
      if (pos1 != std::string::npos || pos2 != std::string::npos) {
      ...
      if (query.size() > 0) {
        ...
        for (size_t i = 0; i < vocabulary.size(); ++i) {
          ...
          if (++numMatches >= overallResults) break;
        }
      ...
      cout << "content string is \"" << content << "\"" << endl;
